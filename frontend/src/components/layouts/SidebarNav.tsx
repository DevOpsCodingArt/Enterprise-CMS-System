      ...
      items: [
        {
          ...
          href: "/company/staff?tab=departments",
          icon: Building,
        },
        {
          id: "staff",
        ...
        },
        {
          ...
          href: "/company/staff?tab=shifts",
          icon: Clock,
        },
        {
          id: "attendance",
          label: "Attendance & Overtime",
          href: "/company/staff?tab=attendance",
          icon: Timer,
        },
        {
          id: "tasks",
          label: "Task Allocation Board",
          href: "/company/staff?tab=tasks",
          icon: CheckSquare,
        },
      ],
    },
      ...
      items: [
        {
          ...
          href: "/company/roles?tab=roles",
          icon: Sliders,
        },
        {
          id: "canned",
          label: "Canned Slash Shortcuts",
          href: "/company/roles?tab=canned",
          icon: Zap,
        },
        {
          id: "sla",
          label: "Working Hours & SLA Rules",
          href: "/company/roles?tab=sla",
          icon: Calendar,
        },
        {
          id: "profile",
          label: "Company Profile & Tax",
          href: "/company/roles?tab=profile",
          icon: Settings,
        },
      ],
    },
        ...
        {navGroups.map((group) => (
          <div key={group.title} className="space-y-1">
            ...
            {group.items.map((item) => {
              ...
              const hasQueryTab = item.href.includes("?tab=");
              const [itemBase, itemQuery] = item.href.split("?tab=");

              let isActive = false;
              if (hasQueryTab) {
                if (currentTab) {
                  isActive = pathname === itemBase && currentTab === itemQuery;
                } else {
                  // Default tab when no query parameter is set
                  isActive =
                    pathname === itemBase &&
                    ((itemBase === "/company/staff" && itemQuery === "departments") ||
                      (itemBase === "/company/roles" && itemQuery === "roles"));
                }
              } else {
                isActive =
                  (pathname === item.href || (item.href !== "/company" && pathname.startsWith(`${item.href}/`))) &&
                  !currentTab;
              }
