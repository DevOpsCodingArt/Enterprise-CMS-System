...
export const conversationInitiatorEnum = pgEnum('conversation_initiator', [
  'customer',
  'staff',
]);
...
export const conversations = pgTable(
  'conversations',
  {
    ...
    customerId: uuid('customer_id')
      ...
      .notNull(),
    initiatedBy: conversationInitiatorEnum('initiated_by')
      .default('customer')
      .notNull(),
  ...
  (table) => [
    index('idx_conversations_company_id').on(table.companyId),
    index('idx_conversations_company_status').on(table.companyId, table.status),
    index('idx_conversations_assigned').on(table.assignedTo, table.status),
    index('idx_conversations_customer').on(table.customerId),
    index('idx_conversations_last_msg').on(
      table.companyId,
      table.lastMessageAt,
    ),
  ],
...
export const messages = pgTable(
  ...
  (table) => [
    ...
    index('idx_messages_status').on(table.conversationId, table.status),
  ],
);

export const chatTransfers = pgTable(
  'chat_transfers',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    conversationId: uuid('conversation_id')
  ...
  (table) => [
    index('idx_transfers_conversation').on(table.conversationId),
    index('idx_transfers_company').on(table.companyId),
  ],
);
...
export const quickReplies = pgTable(
  ...
  (table) => [
    index('idx_quick_replies_company').on(table.companyId),
    index('idx_quick_replies_shortcut').on(table.companyId, table.shortcut),
  ],
);
...
export const workingHours = pgTable(
  ...
  (table) => [index('idx_working_hours_company').on(table.companyId)],
);
