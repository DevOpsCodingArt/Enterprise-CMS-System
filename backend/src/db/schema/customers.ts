...
export const customerClassEnum = pgEnum('customer_class', [
  'residential',
  'business',
  'corporate',
  'government',
...
export const customers = pgTable(
  'customers',
  {
    ...
    customerClass: customerClassEnum('customer_class')
      .default('residential')
      .notNull(),
    packageId: varchar('package_id', { length: 100 }),
    packageName: varchar('package_name', { length: 255 }).default(
      '50 Mbps Unlimited Fiber',
    ),
    packageSpeed: varchar('package_speed', { length: 50 }).default('50 Mbps'),
    monthlyBilling: decimal('monthly_billing', {
      precision: 10,
      scale: 2,
    }).default('3500.00'),
    ...
    onuSignalDbm: decimal('onu_signal_dbm', { precision: 5, scale: 2 }).default(
      '-19.50',
    ),
  ...
  (table) => [
    ...
    index('idx_customers_company_status').on(table.companyId, table.status),
    index('idx_customers_company_branch').on(table.companyId, table.branchId),
    uniqueIndex('idx_customers_company_code').on(
      table.companyId,
      table.customerCode,
    ),
    index('idx_customers_phone').on(table.phone),
    index('idx_customers_cnic').on(table.cnic),
    index('idx_customers_username').on(table.username),
    index('idx_customers_email').on(table.email),
    index('idx_customers_billing_expiry').on(
      table.companyId,
      table.billingExpiryDate,
    ),
  ],
);
